...
describe("EarnNFt", function () { 
    ...
    describe("test EarnNft minting", function () {
        it("Should fail when not enough balance", async function () {
            ...
            await expect(earnNFT.connect(firstAccount).mint()).to.be.revertedWith("EarnNFT: not enough money");
        });
        ...
        it("Should get correct information after minting", async function () {
            const { earnNFT, firstAccount, baseUri } = await loadFixture(getContracts);
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
    ...
    describe("test EarnNft merging", function () {
        it("Should fail when merge is called with no owner of the tokens", async function () {
            ...
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(secondAccount).mint({value: ethers.utils.parseEther('0.01')});
        ...
        it("Should get the RARE nft token", async function () {
            const { earnNFT, firstAccount, secondAccount } = await loadFixture(getContracts);
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
        ...
        it("Should get the EPIC nft token", async function () {
            const { earnNFT, firstAccount, secondAccount } = await loadFixture(getContracts);
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
        ...
        it("Should fail while merging Rare and UNCOMMON", async function () {
            const { earnNFT, firstAccount, secondAccount } = await loadFixture(getContracts);
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
            await earnNFT.connect(firstAccount).mint({value: ethers.utils.parseEther('0.01')});
